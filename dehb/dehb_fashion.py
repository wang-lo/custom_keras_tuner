...
def run_experiment(x, y, n_splits=3, n_inner_splits=3, max_epochs=50, max_trials=100, seed=42):
    ...
    accuracy_logger = AccuracyLogger(f"dehb_fashion_15_accuracy_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv")
    ...
    for fold, (train_index, test_index) in enumerate(outer_cv.split(x,y), 1):
        ...
        for inner_fold, (inner_train_index, inner_val_index) in enumerate(inner_cv.split(x_train, y_train), 1):
            ...
            project_name=f'fashion_mnist_dehb_fold_{fold}_{inner_fold}'
            ...
            logger.info(f"Fold {fold}.{inner_fold} completed in {inner_time} with test accurcy: {val_accuracy} and config: {best_hp.get_config()['values']}")
