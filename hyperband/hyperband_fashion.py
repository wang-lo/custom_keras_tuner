...
log_filename = f"fh8_tuning_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
class HyperbandHyperModel(kt.HyperModel):
    def build(self, hp):
...
class LoggingHyperband(kt.Hyperband):
...
def run_experiment(x, y, n_splits=3, n_inner_splits=3, max_epochs=50, max_trials=100, seed=42):
    ...
    accuracy_logger = AccuracyLogger(f"fh8_accuracy_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv")
    ...
    for fold, (train_index, test_index) in enumerate(outer_cv.split(x,y), 1):
        ...
        for inner_fold, (inner_train_index, inner_val_index) in enumerate(inner_cv.split(x_train, y_train), 1):
            ...
            tuner_dir = f'hyperband_tuning_fold_{fold}_inner_{inner_fold}'
            project_name = f'fashion_mnist_hyperband_fold_{fold}_{inner_fold}'

            hypermodel = HyperbandHyperModel()
            inner_tuner = LoggingHyperband(
            ...
            inner_tuner.search(x_inner_train, y_inner_train, validation_data=(x_val, y_val), epochs=max_epochs)  
