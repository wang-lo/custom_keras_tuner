...
class BOHBHyperModel(kt.HyperModel):
...
class BOHBOracle(kt.Oracle):
    def __init__(self, objective, searcher: BOHB, max_trials):
...
class BOHBTuner(kt.Tuner):
    def __init__(self, hypermodel, config_space, objective, max_epochs, max_trials, seed, accuracy_logger, fold, inner_fold, directory=None, project_name=None, **kwargs):
        if directory is None:
            directory = 'bohb_tuning'
        if project_name is None:
            project_name = 'mnist_bohb'
        ...
        self.bohb = BOHB(
            ...
            max_t=max_epochs,
            random_seed=seed)
        self.oracle = BOHBOracle(objective, searcher=self.bohb, max_trials=max_trials)
    ...
    def on_epoch_end(self, trial, model, epoch, logs=None):
        ...
        decision = self.bohb.on_trial_result(trial=self.curr_trial, result=result)
        ...
        if decision == 'STOP':
            ...
            self.bohb.on_trial_complete(self.curr_trial, result=result)
...
def run_experiment(x, y, n_splits=3, n_inner_splits=3, max_epochs=50, max_trials=100, seed=42):
    ...
    for fold, (train_index, test_index) in enumerate(outer_cv.split(x,y), 1):
        ...
        for inner_fold, (inner_train_index, inner_val_index) in enumerate(inner_cv.split(x_train, y_train), 1):
            ...
            tuner_dir = f'bohb_tuning_fold_{fold}_inner_{inner_fold}'
            project_name=f'fashion_mnist_bohb_fold_{fold}_{inner_fold}'

            hypermodel = BOHBHyperModel()
            inner_tuner = BOHBTuner(
...
run_experiment(x=x, y=y, n_splits=3, n_inner_splits=3, max_epochs=50, max_trials=100, seed=15)
