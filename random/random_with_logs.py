...
log_filename = f"random_8_tuning_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
def run_experiment(x, y, n_splits=3, n_inner_splits=3, max_epochs=50, max_trials=100, seed=42):
    ...
    logger.info(f"Starting experiment with {n_splits} outer splits and {n_inner_splits} inner splits seed {seed} and db mnist")
    accuracy_logger = AccuracyLogger(f"random_8_accuracy_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv")
    ...
    for fold, (train_index, test_index) in enumerate(outer_cv.split(x,y), 1):
        ...
        for inner_fold, (inner_train_index, inner_val_index) in enumerate(inner_cv.split(x_train, y_train), 1):
            ...
            project_name = f'mnist_random_search_fold_{fold}_{inner_fold}'
